...
if(btnSliderRight != null && btnSliderLeft != null){
btnSliderLeft.addEventListener("click", function(){
    if(SlideCount != 4){
    SlideCount++;
    document.querySelector(".big-ad img").src = `Img/slider${SlideCount}.jpg`;
    }else{
        SlideCount = 1;
        document.querySelector(".big-ad img").src = `Img/slider${SlideCount}.jpg`;
    }
    ...
    loginBtn.addEventListener("click",function(){
           fetch('/json/users.json')
            .then(response => response.json())
             .then(products => {
                products.forEach(product =>{
                    console.log(product.Mail,product.password);
                    if(MailInput.value == product.Mail && PasswordInput.value == product.password){
